...
describe('validateModelIdExistsMiddleware', () => {
  ...
  test('should return 400 if id is not provided', async () => {
    ...
    const middleware = validateModelIdExistsMiddleware(modelService as ModelServiceInterface);
  ...
  test('should return 404 if model with id is not found', async () => {
    ...
    expect(res.json).toHaveBeenCalledWith({ message: 'model with id 1 not found' });
    expect(next).not.toHaveBeenCalled();
  });
