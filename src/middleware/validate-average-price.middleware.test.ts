...
describe('validateAveragePriceMiddleware', () => {
  ...
  test('should return 400 if average price is not provided', () => {
    ...
    expect(res.status).toHaveBeenCalledWith(400);
    expect(res.json).toHaveBeenCalledWith({ message: 'you must provide an average price!' });
    expect(next).not.toHaveBeenCalled();
  });
  ...
  test(`should return 400 if average price is less than ${MIN_AVG_PRICE}`, () => {
    ...
    expect(res.status).toHaveBeenCalledWith(400);
    expect(res.json).toHaveBeenCalledWith({ message: `average price invalid, must be ${MIN_AVG_PRICE} at least` });
    expect(next).not.toHaveBeenCalled();
  });
