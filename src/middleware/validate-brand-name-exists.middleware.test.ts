...
describe('validateBrandNameExistsMiddleware', () => {
  ...
  test('should return 400 if name is not provided', async () => {
    ...
    const middleware = validateBrandNameExistsMiddleware(brandService as BrandServiceInterface);

    await middleware(req as Request, res as Response, next);

    expect(res.status).toHaveBeenCalledWith(400);
    expect(res.json).toHaveBeenCalledWith({ message: 'you must provide a name!' });
  ...
  test('should return 409 if brand with name already exists', async () => {
    ...
    expect(res.json).toHaveBeenCalledWith({ message: 'brand with name ExistingBrand already exists' });
    expect(next).not.toHaveBeenCalled();
  });
