...
export const validateBrandNameExistsMiddleware = (brandService: BrandServiceInterface) => {
  return (async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (model.length) {
      res.status(409).json({ message: `brand with name ${req.body.name} already exists` });
