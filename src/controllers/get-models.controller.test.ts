...
describe('getModelsController', () => {
  ...
  test('should return filtered models and status 200', async () => {
    ...
    const controller = getModelsController(modelService as ModelServiceInterface);

    await controller(req as Request, res as Response);

    expect(modelService.getModels).toHaveBeenCalledWith({ greater: '100000', lower: '200000' });
  ...
  test('should return 500 status if an error occurs', async () => {
    ...
    (modelService.getModels as jest.Mock).mockRejectedValue(new Error(errorMessage));
